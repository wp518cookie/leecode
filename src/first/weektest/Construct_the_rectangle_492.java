...
public class Construct_the_rectangle_492 {
    ...
    public static boolean PredictTheWinner(int[] nums, int start, int end, int value, boolean flag) {
        ...
        if (flag) {
            return (PredictTheWinner(nums, start + 1, end, value + nums[start], false) || PredictTheWinner(nums, start, end - 1, value + nums[end], false));
        } else {
            return (PredictTheWinner(nums, start + 1, end, value - nums[start], true) && PredictTheWinner(nums, start, end - 1, value - nums[end], true));
        }
    }
}
