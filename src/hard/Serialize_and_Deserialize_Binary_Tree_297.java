...
//        treeNode1.left = treeNode2;
    ...
    public static String serialize(TreeNode root) {
        ...
        TreeNode[] treeNodes = getVal(root, getDepth(root));
        int count = 0;
        ...
        for (int i = 0; i < treeNodes.length-count; i++) {
            if (treeNodes[i] != null) {
                result[i] = String.valueOf(treeNodes[i].val);
            }
        }
    }
    ...
    public static TreeNode deserialize(String data) {
        ...
        int depth = 0;
        int length = data_arr.length;
        while (length / 2 > 0) {
            length = length / 2;
            depth++;
        }
        TreeNode[] treeNodes = new TreeNode[(2 << depth) - 1];
        for (int i = 0; i < data_arr.length; i++) {
            if (data_arr[i] != null && !data_arr[i].equals("null")) {
                TreeNode treeNode = new TreeNode(Integer.valueOf(data_arr[i]));
    ...
    public static TreeNode[] getVal(TreeNode root, int depth) {
        TreeNode[] result = new TreeNode[(2 << depth) - 1];
        int count = 0;
        int index = 0;
        while (count <= depth) {
            if (count == 0) {
                ...
                while (index < (2 << count) - 1) {
                    if (count == 1) {
                        result[index++] = result[0] == null ? null : result[0].left;
                        result[index++] = result[0] == null ? null : result[0].right;
                    } else {
                        int temp_index = index;
                        result[index++] = result[temp_index / 2] == null ? null : result[temp_index / 2].left;
                        result[index++] = result[temp_index / 2] == null ? null : result[temp_index / 2].right;
                    }
                }
            }
            count++;
